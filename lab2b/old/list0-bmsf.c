...
void setupLocks(int nmutex, int nspinlocks){

    mutexes = malloc(sizeof(pthread_mutex_t) * nmutex);
    ...
    spinLocks = malloc(sizeof(int) * nspinlocks);
    for (int i = 0; i < nspinlocks; i++)
    	spinLocks[i] = 0;
...
void initializeSubLists(){
    lists = malloc(sizeof(SortedList_t) * 1);
    for (int i = 0; i < 1; i++){
