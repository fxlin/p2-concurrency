...
void* thread_func(void* thread_id){
	...
	for (int i = per_part * id; i < per_part * (id + 1); i++) {
		// TODO: must update this insertion strategy to use more than one hash table
		// to convert to the "big table" approach. Right now it currently uses a single
		// hash table and lock.
		assert(index>=0);
		pthread_mutex_lock(&mutexes[0]);
...
int main(int argc, char** argv) {
	...
	// TODO: Must update to allocate N hash tables for "big table" approach.
	numHashTables = 1;
